...
async def local(num: int) -> AsyncIterable[BgData]:
    ...
    for x in files:
        yield BgFileData(
            x,
            mimetypes.guess_type(str(x))[0] or DEFAULT_MIME,
        )
...
def cache_bg(bg: BgBytesData):
    ...
    ext = bg.mime.split("/")[-1] if "/" in bg.mime else "bin"
    path = BG_PRELOAD_CACHE_DIR / f"{time.time_ns()}.{ext}"
...
def read_cached_bg_file(bg: BgFileData) -> BgBytesData | None:
    ...
    try:
        ...
        try:
            if str(bg.path).startswith(str(BG_PRELOAD_CACHE_DIR)):
                bg.path.unlink()
        except Exception:
            pass
...
class BgPreloader:
    ...
    async def preload_task(
        ...
        logger.debug(f"Preload task started, will preload {count} images, fire={fire}")
        try:
            async for x in fetch_bg(count):
                logger.debug("Got one image")
                if self.preload_count > 0 or (fire_done_signal and fire_done_signal.is_set()):
        ...
        if fire:
            return
        if self.consumed_in_loading or self.background_queue.qsize() < self.preload_count:
    ...
    def start_preload(self, force: bool = False):
        ...
        if count <= 0 and not force:
            logger.debug("Current background queue size meets preload count, skip preload")
            return
        task = aio.create_task(self.preload_task(max(count, 1)))
    ...
    async def _get_on_fire(self) -> BgBytesData:
        ...
        try:
            await aio.wait_for(task_done_signal.wait(), timeout=15)
