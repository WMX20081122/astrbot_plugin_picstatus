...
class BasePeriodicCollector(Collector[T, deque[T]], Generic[T]):
    def __init__(self, size: int = None) -> None:
        super().__init__()
        size = size or config.ps_default_collect_cache_size
...
def functional_collector(cls: type[Collector], name: str | None = None):
    def deco(func: TCF) -> TCF:
        ...
        class _Collector(cls):
            async def _get(self) -> Any:
                return await func()

        collector(collector_name)(_Collector)
        return func
    return deco
...
class BaseTimeBasedCounterCollector(Collector[R, Any], Generic[T, R]):
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.last_obj = Undefined()
        self.last_time = 0.0
        self.normal_delay = 1.0
...
def load_builtin_collectors():
    for module in Path(__file__).parent.iterdir():
        if not module.name.startswith("_") and module.suffix == ".py":
...
async def setup_periodic_collectors_update_job():
    ...
    async def _loop():
        while True:
            ...
            await asyncio.sleep(config.ps_collect_interval)
